...
export function fetchIntegrationSettings(): IntegrationSettings {
  try {
    const raw = localStorage.getItem('schedulify_settings');
    if (!raw) {
...
export function saveIntegrationSettings(settings: IntegrationSettings): void {
  localStorage.setItem('schedulify_settings', JSON.stringify(settings));
...
export async function fetchEventTypes(): Promise<EventType[]> {
  ...
  const raw = localStorage.getItem('schedulify_event_types');
  if (!raw) {
    localStorage.setItem('schedulify_event_types', JSON.stringify(DEFAULT_EVENT_TYPES));
    return DEFAULT_EVENT_TYPES;
  }
  return JSON.parse(raw);
}
...
export async function saveEventType(event: EventType): Promise<void> {
  ...
  localStorage.setItem('schedulify_event_types', JSON.stringify(events));
}
...
export async function deleteEventType(id: string): Promise<void> {
  ...
  localStorage.setItem('schedulify_event_types', JSON.stringify(filtered));
}
...
export async function fetchAvailability(): Promise<Availability> {
  ...
  // Local Storage
  const raw = localStorage.getItem('schedulify_availability');
  if (!raw) {
    localStorage.setItem('schedulify_availability', JSON.stringify(DEFAULT_AVAILABILITY));
    return DEFAULT_AVAILABILITY;
  }
  return JSON.parse(raw);
}
...
export async function saveAvailability(availability: Availability): Promise<void> {
  ...
  // Local Storage
  localStorage.setItem('schedulify_availability', JSON.stringify(availability));
}
...
export async function fetchBookings(): Promise<Booking[]> {
  ...
  // Local Storage
  const raw = localStorage.getItem('schedulify_bookings');
...
export async function saveBooking(booking: Omit<Booking, 'id' | 'createdAt' | 'status'>): Promise<Booking> {
  ...
  localStorage.setItem('schedulify_bookings', JSON.stringify(bookings));
  return newBooking;
}
